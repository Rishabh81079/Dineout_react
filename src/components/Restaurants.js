...
function Restaurants({carddata}){
    return(
        ...
        <div className="font-serif transform transition hover:scale-95 duration-200 w-60">
        ...
        <h1 className="font-bold overflow-hidden ">{carddata?.info?.name}</h1>
        <h1 className="text-base font-medium">{carddata?.info?.avgRating}</h1>
        <h1 className="text-sm text-gray-500 overflow-hidden">{carddata?.info?.cuisines.join(',')}</h1>
